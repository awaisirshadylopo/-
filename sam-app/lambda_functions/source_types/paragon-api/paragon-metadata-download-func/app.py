...
def create_token(client_id, client_secret, loginUrl, sid, cursor):
    ...
    if response.status_code == 200:
        ...
        creds = f"select id, name, auth->'loginUrl' as loginurl, '{token}' as token, auth->'user' as user, auth->'logoutUrl' as logouturl, source_info->'source_type' from public.source where source_info::text ~* 'Paragon' and id = {sid};"
...
def creds_from_source_table(connection, cursor, sid):
    creds = f"select id, name, auth->'loginUrl' as loginurl, auth->'password' as token, auth->'user' as user, auth->'logoutUrl' as logouturl, source_info->'source_type' from public.source where source_info::text ~* 'Paragon' and id = {sid};"
    ...
    cursor.execute(creds)
    source_creds = cursor.fetchall()
    if source_creds[0][6] == 'Paragonrels API':
        source_creds = create_token(source_creds[0][4], source_creds[0][3], source_creds[0][5], sid, cursor)
        return source_creds
    elif source_creds[0][6] == 'Paragon API':
        return source_creds
...
def DownloadMetaData(metadata_url, token):
    ...
    for all_tree in root:
        for inner_tree in all_tree:
            if 'Namespace' in inner_tree.attrib and inner_tree.attrib['Namespace'] == 'ODataService': # For Paragonrels, the namespace was Paragon. Change accordingly.
...
def ddl_generation(source_id, connection, cursor):
    ...
    for each_table in ps_tables_name:
        ...
        table = f"idx_stage.ps_paragon_{each_table}_{source_id}" if each_table != 'Media' else f"idx_stage.ps_paragon_{each_table}"
...
def lambda_handler(event, context):
    try:
        ...
        classList, list_dict, status_code = DownloadMetaData(credentials[0][2], credentials[0][3])
