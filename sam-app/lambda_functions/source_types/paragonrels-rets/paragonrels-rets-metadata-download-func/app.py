...
logger = logging.getLogger("mls-paragonrels-rets-metadata-download")
logger.setLevel(logging.INFO)

class HostHeaderSSLAdapter(HTTPAdapter):
    """Transport adapter that allows us to override the DNS resolution."""
...
def login(data):
  ...
  if source_id in [798, 604]:
    response,session = login_rets(login_url, username, password, USER_AGENT)
  else:
    response = session.get(login_url)
...
def resource_metadata(data):
    ...
    logger.info("Resource Metadata Started")
    source_id = data['source_id']
    source_name = data['name']
...
def field_metadata(data, df):
    ...
    logging.info(log_msg)
    field_df = field_df.drop_duplicates(subset=['Long_Name','resource_name','class_name'], keep='first')
    return field_df
...
def ddl_generation(source_id, connection, cursor):
    ...
    for res_name in ps_tables:
        ...
        if res_name[0] in 'Property':
            ...
            # Properly quote column names that might contain special characters
            ddl_names = [f'"{j[0]}" text NULL' for j in ddl_ if j[0]]  # Added quotes around column names
...
def lambda_handler(event, context):
    ...
    try:
        ...
        for i, item in source_list.iterrows():
            ...
            auth = item['auth']
            if 'paragonrels' in auth['loginUrl']:
                rets_data['Login'] = auth['loginUrl']
