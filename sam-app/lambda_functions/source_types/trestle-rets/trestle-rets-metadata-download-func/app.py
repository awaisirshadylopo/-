...
logger = logging.getLogger("mls-trestle-rets-metadata-download")
...
def login(data):
    ...
    # def login(data):
    username= data['user']
    if isinstance(data.get('headers'), (list, dict)):
        pass
    else:
        data['headers'] ={
    ...
    response = requests.get(loginUrl, auth=auth)
    ...
    # Send login request
    # response = session.get(loginUrl, auth=auth)
    
    # Check for successful login
    try:        
        root = ET.fromstring(response.text)
        reply_code = root.get('ReplyCode')
        if reply_code == '0':
            log_msg={
                'Level':'INFO',
...
def resource_metadata(data):
    ...
    metadata_response = requests.get(metadata_url, params=Metadata_params, auth=auth)
...
def field_metadata(data, df):
    ...
    for i, row in df.iterrows():
        ...
        Metadata_params = {
            ...
            'ID': class_name,          # Replace with '0' for all, or a specific ID (e.g., Property, RE_1)
            'Format': 'COMPACT'
        }
        metadata_response = requests.get(metadata_url, params=Metadata_params, auth=auth)
    ...
    field_df = field_df[['source_id','source_name','resource_name','class_name','Long_Name','renamed_long_name','DB_Name','System_Name','Max_Length','DataType']]
    log_msg = {
            'Level':'Info',
            'Function': 'field_metadata()',
