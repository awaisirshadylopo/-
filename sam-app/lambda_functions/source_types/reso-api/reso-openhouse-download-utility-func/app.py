"""Reso API OpenHouse Download Lambda"""
...
def download_all_openhouse(source_data, cursor_rds):
    ...
    top = source_data["source_info"]["top"]

    current_datetime = datetime.now()
    one_day_ago = current_datetime - timedelta(days=1)
    one_day_ago_formatted = one_day_ago.strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    select_fields = "ListingKey,OpenHouseStartTime,OpenHouseEndTime"
    if source_id in (620, 761, 957):
        select_fields = select_fields + ",OpenHouseDate"
    elif source_id == 968:
        select_fields = select_fields.replace("ListingKey", "ListingId")
    ...
    if source_id in (968, 639):
        tokenurl = source_data["auth"]["tokenUrl"]
        username = source_data["auth"]["username"]
        password = source_data["auth"]["password"]
        client_id = source_data["auth"]["client_id"]
        client_secret = source_data["auth"]["client_secret"]
