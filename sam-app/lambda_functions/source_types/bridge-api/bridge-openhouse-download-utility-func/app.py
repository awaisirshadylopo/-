"""Bridge API Validation and Data Download"""
    ...
    try:
        ...
        if source_id == 613:
            ...
            CREATE TEMP TABLE IF NOT EXISTS {temp_table} (
                ...
                ListingKey TEXT,
                openhousedate Date,
                VALLEYMLS_LocalStartTime TEXT,
                VALLEYMLS_LocalEndTime TEXT, 
                y_creation_date timestamp, 
                y_update_date timestamp
            ) ON COMMIT PRESERVE ROWS;
            """
        ...
        if source_id == 613:
            insert_temp_query = f"""
            INSERT INTO {temp_table}
            (source_id, batch_id, ListingKey, openhousedate, VALLEYMLS_LocalStartTime, VALLEYMLS_LocalEndTime, y_creation_date, y_update_date)
            VALUES (%(source_id)s, %(batch_id)s, %(ListingKey)s, %(openhousedate)s, %(VALLEYMLS_LocalStartTime)s, %(VALLEYMLS_LocalEndTime)s, %(y_creation_date)s, %(y_update_date)s)
            """
        # Execute the insert
        try:
        ...
        logger.error(
            {
                ...
                "source_id": source_id,
                "batch_id": batch_id,
                # "insert dynamic query": insert_dynamic_query,
            }
        )
        raise
...
def download_all_Openhouse_func(
    ...
    Created:    2026-01-07                  Create By : Ammar Azkar
    """
    ...
    # originating_system_name = repr(source_data["source_info"]["originating_system_name"].replace("'",""))
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    top = 200  # Number of records per page
    ...
    if source_id == 613:
        params["$select"] = (
            f"ListingKey,OpenHouseDate,VALLEYMLS_LocalStartTime,VALLEYMLS_LocalEndTime"
        )
    if source_id in (709, 723, 738, 767, 873, 913, 924, 946):
        params["$select"] = f"ListingKey,OpenHouseStartTime,OpenHouseEndTime"

    next_url = url_endpoint

    logger.info({"step": "API_REQUEST", "endpoint": url_endpoint, "params": params})
    # -------------------------------------------------------------
    # 4. Call Bridge API and handle OData pagination
    # -------------------------------------------------------------
