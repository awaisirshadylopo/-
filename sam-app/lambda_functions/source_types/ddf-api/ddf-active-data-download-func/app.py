# DDF active data download lambda
...
logger = logging.getLogger("DDF-Active-Data-Download-Lambda")
...
def create_token(client_id, client_secret):
    ...
    data = {"grant_type": "client_credentials", "scope": "DDFApi_Read"}
    ...
    try:
        ...
        for resource_name in resources:
            ...
            if resource_name == "Property":
                ...
                if len(property_df) != 0:
                    ...
                    for expanded_class in expand_classes:
                        ...
                        if "media" in expanded_class.lower():
                            ...
                            expanded_class = "PropertyRoom"

                        table_name = f"ps_ddf_{expanded_class}"
                ...
                # office/agent key/mlsids chunks creation from Property's columns
                ...
                total_count = top = 100  # initial dummy value
                skip = 0
                params = {
                    "$filter": f"OpenHouseDate ge {current_date}",
                    "$top": top,
                    "$skip": skip,
                    "$orderby": "OpenHouseDate",
                    "$count": "true",
                }
            ...
            if len(data_df) != 0:
                ...
                prestage_tables_insertion(
                    ...
                    source_name,
                    resource_name,
                    f"ps_ddf_{resource_name}_{source_id}",
                    cursor_rds,
                    rds_connection,
                )
    except:
...
def request_and_load_temp_table(source_data, flow_type, cursor_rds, rds_connection):
    ...
    params = {
        "$filter": f"ModificationTimestamp ge {last_modified_date}",
        "$count": "true",
    ...
    if len(temp_table_df) != 0:
        ...
        temp_table_df.insert(0, "source_id", int(source_data["source_id"]))
...
def validation_func(source_data, cursor_homelisting, cursor_rds, rds_connection):
    ...
    try:
        ...
        flow_type = "lmd"  # only active data; hence hard-coding flow_type
