...
def creds_from_source_table(connection, cursor, sid):
    creds = f"select id, name, auth->'loginUrl' as loginurl, auth->'user' as client_id, auth->'password' as client_secret from public.source where source_info::text ~* 'DDF' and id = {sid};"
...
def create_token(client_id, client_secret):
    ...
    if response.status_code == 200:
        ...
        log_message(LogMessage("INFO", "token genrated", log_data))
        return token
    else:
        log_data = LogData(event=response)
        log_message(LogMessage("ERROR", "token generation failed", log_data))
    ...
    for all_tree in root:
        for inner_tree in all_tree:
            if (
                ...
                and inner_tree.attrib["Namespace"] == "DDF.Core.Entities"
            ):
...
def ddl_generation(source_id, connection, cursor):
    ...
    for each_table in ps_tables_name:
        ...
        final_query = (
            """CREATE TABLE IF NOT EXISTS idx_stage.ps_ddf_{0}_{2} ( {1} """.format(
...
def lambda_handler(event, context):
    try:
        ...
        metadata_insertion(
            classList, list_dict, source_name, source_id, rds_conn, rds_cursor
        )
