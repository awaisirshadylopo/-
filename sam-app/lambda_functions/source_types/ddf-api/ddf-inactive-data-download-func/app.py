...
def get_total_count_from_property_endpoint(base_property_url, token):
    ...
    # "$filter": "ModificationTimestamp ge 1990-01-01T00:00:00Z and StandardStatus eq 'Active'",
    # DDF API doesn't allowing to filter on Status; has only active data; Modification Timestamp filter also not required because it always needs to fetch from 1990 in inactive data download
    params = {
        "$count": "true",
        "$top": 1,
        "$select": "ListingKey",
