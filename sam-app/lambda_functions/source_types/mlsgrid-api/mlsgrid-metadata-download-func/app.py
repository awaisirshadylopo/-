    ...
    for all_tree in root:
        for inner_tree in all_tree:
            if (
                ...
                and inner_tree.attrib["Namespace"] == "com.mlsgrid.metadata"
            ):
...
def ddl_generation(source_id, connection, cursor):
    ...
    for each_table in ps_tables_name:
        ...
        table = (
            f"idx_stage.ps_mlsgrid_{each_table}_{source_id}"
            if each_table != "Media"
            else f"idx_stage.ps_mlsgrid_{each_table}"
        )
...
def lambda_handler(event, context):
    try:
        ...
        classList, list_dict, status_code = DownloadMetaData(
            credentials[0][2], credentials[0][3]
        )
