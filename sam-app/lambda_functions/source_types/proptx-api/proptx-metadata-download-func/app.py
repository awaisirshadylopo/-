...
def creds_from_source_table(connection,cursor, sid):
    creds = f"select id, name, auth->'loginUrl' as loginurl, auth->'password' as client_secret from public.source where source_info::text ~* 'proptx' and id = {sid};"
...
def ddl_generation(source_id,connection,cursor):
    ...
    for each_table in ps_tables_name:
        ...
        final_names = str(final_names).replace('[','').replace(']','').replace('\'','').replace('Order','order')

        final_query = '''CREATE TABLE IF NOT EXISTS idx_stage.ps_proptx_{0}_{2} ( {1} '''.format(each_table,final_names,source_id)
...
def lambda_handler(event, context):
    try:
        ...
        bearer_token = credentials[0][3]
        
        classList,list_dict, status_code = DownloadMetaData(loginurl,bearer_token)
