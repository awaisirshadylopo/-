...
logger = logging.getLogger("mls-flexmls-rets-inactive-data-download-func")
...
def download_listing_for_inactive(cursor, rds_cursor, connection, event, response):
    ...
    #     listing_chunk = ', '.join(listing_chunk) # str(listing_chunk).replace('[', '').replace(']', '').replace("'", '"')
    #     query = f"({inactive_key_field}= {listing_chunk})"
    query = f"({modification_column}=1990-01-01+)({status_column}=|{active_status})"
    for r in results:
        query_params = {
            "SearchType": "Property",
            "Class": r,
