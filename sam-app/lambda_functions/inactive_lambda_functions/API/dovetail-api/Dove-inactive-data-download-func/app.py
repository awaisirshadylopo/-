...
def api_call_and_load_tables(
    ...
    df = df.drop(columns=["@odata.id", "@odata.etag"], errors="ignore")
    df = df.rename(
...
def lambda_handler(event, context):
    ...
    token_generation = event["source_info"].get("token_generation", False)
    # GENERATES TOKEN
    if token_generation:
        token = create_token(source_id, source_name, client_id, token)
        event["token"] = token
    else:
        event["token"] = token
