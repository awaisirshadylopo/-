...
def download_and_update_sold_date(cursor, rds_cursor, connection, event, response):
    ...
    (sold_date is null or sold_price is null) 
    ...
    for listing_chunk in inactive_listings_chunks:
        listing_chunk = ",".join(listing_chunk)
    ...
    query = """
        update listing_p_sold set sold_date = %s , sold_price = %s where source_id  =  %s and  source_listing_id = %s
        """

    data_to_update = [
        tuple(row)
        for row in property_df[
            ["sold_date", "sold_price", "source_id", "source_listing_id"]
        ].values
    ]
