"""PROPTX API Validation and Data Download"""
...
def download_all_Openhouse_func(
    ...
    """
    Description:
        Download open house records from Bridge API using $top and $skip pagination.
    """
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 2. Build Bridge OpenHouse API endpoint and headers
    # -------------------------------------------------------------
    ...
    last_day_time = (datetime.now(timezone.utc) - timedelta(days=1)).strftime("%Y-%m-%d")
    
    # Pagination parameters
    top = 100  # Records per page (adjust based on API limits, max usually 500)
    skip = 0   # Starting offset
    all_openhouse_data = []
    page_count = 1
    
    # URL encode the filter
    filter_string = f"OpenHouseDate ge {last_day_time}"
    encoded_filter = quote(filter_string, safe='')
    
    logger.info({
        "step": "API_PAGINATION_START",
        "source_id": source_id,
        "batch_id": batch_id,
        "top": top,
    ...
    # -------------------------------------------------------------
    # 3. Loop using $top and $skip until no more records
    # -------------------------------------------------------------
    while True:
        try:
            # Build URL with $top and $skip
            full_url = (f"{url_endpoint}?$filter={encoded_filter}"
                       f"&$select=ListingKey%2COpenHouseDate%2COpenHouseStartTime%2COpenHouseEndTime"
                       f"&$top={top}&$skip={skip}")
            ...
            # Make API request
            response = requests.get(url=full_url, headers=headers, timeout=30)
            ...
            # Add to collection
            all_openhouse_data.extend(records)
            
            logger.info({
                "step": "API_PAGE_RESPONSE",
                "page": page_count,
                "records_in_page": records_count,
                "total_records_so_far": len(all_openhouse_data),
                "skip_used": skip
            })
            
            # Check if we got fewer records than requested (last page)
            if records_count < top:
                logger.info({
                    "step": "PAGINATION_COMPLETE",
                    "message": "Last page reached",
                    "total_pages": page_count,
                    "total_records": len(all_openhouse_data)
                })
                break
            ...
            # Safety check to prevent infinite loops (max 1000 pages = 100,000 records)
            if page_count > 1000:
                logger.warning("Reached maximum page limit (1000 pages). Stopping pagination.")
                break
                
        except requests.exceptions.RequestException as e:
            logger.error(f"Request failed on page {page_count}: {e}")
            
            # Retry logic for failed page
            if page_count <= 3:  # Retry first 3 pages only
                logger.info(f"Retrying page {page_count}...")
                time.sleep(5)
                try:
                    response = requests.get(url=full_url, headers=headers, timeout=30)
                    response.raise_for_status()
    ...
    # -------------------------------------------------------------
    # 4. Log final download statistics
    # -------------------------------------------------------------
    logger.info({
        "source_id": source_id,
        "batch_id": batch_id,
        ...
        "total_pages": page_count,
        "total_records_downloaded": len(all_openhouse_data),
        "final_skip_value": skip
    })
    
    # -------------------------------------------------------------
    # 5. Process and insert data
    # -------------------------------------------------------------
    if all_openhouse_data:
        ...
        source_data["Openhouse_records_downloaded"] = len(all_openhouse_data)
        ...
        logger.info({
            ...
            "message": "No OpenHouse records found for the last 1 day.",
        })
        source_data["Openhouse_records_downloaded"] = 0
        source_data["Openhouse_records_inserted"] = 0
