"""Unified RETS Inactive OpenHouse Data Download Lambda"""
...
def request_source(data):
    ...
    query_params.setdefault("QueryType", "DMQL2")
    query_params.setdefault("Format", "COMPACT-DECODED")
    query_params.setdefault("Count", "1")
...
def download_all_openhouse(source_data, cursor_rds):
    ...
    if source_id == 683:
        select_fields = f"ListingId,Modified,StartDt,StartTm,EndTm"
        hitting_query = f"(StartDt={one_day_ago_formatted}+)"
    elif source_id==777:
        select_fields = (
            f"ListNumS,OpenHouseDate,StartTime,EndTime"
        )
        hitting_query = f"(OpenHouseDate={one_day_ago_formatted}+)"

    else:
        select_fields = (
            f"ListingId,OpenHouseDate,OpenHouseStartTime,OpenHouseEndTime"
        )
        hitting_query = f"(OpenHouseDate={one_day_ago_formatted}+)"
    ...
    if source_id in (683,777):
        query_params["QueryType"] = "DMQL"
    else:
        query_params["QueryType"] = "DMQL2"
