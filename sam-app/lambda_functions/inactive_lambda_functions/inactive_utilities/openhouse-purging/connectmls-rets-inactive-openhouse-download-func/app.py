"""ConnectMLS Rets Inactive OpenHouse Download Lambda"""
...
def download_all_openhouse(source_data, cursor_rds):
    ...
    if source_id in (298,):
        select_fields = f"MLSNO,START,START,END"
        hitting_query = f"(START={one_day_ago_formatted}+)"
    elif source_id in (827,):
        select_fields = f"OHListingKey,OHDate,OHStartTime,OHEndTime"
        hitting_query = f"(OHDate={one_day_ago_formatted}+)"
    else:
        select_fields = f"OHListingId,OHDate,OHStartTime,OHEndTime"
        hitting_query = f"(OHDate={one_day_ago_formatted}+)"
    query_params = {
