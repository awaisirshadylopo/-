"""Paragonrels API Validation and Data Download"""
    ...
    try:
        ...
        if source_id in (904, 981):
            ...
            CREATE TEMP TABLE IF NOT EXISTS {temp_table} (
                ...
                ListingKey TEXT,
                openhousedate Date,
                Start_Date_Or_Time TEXT,
                Event_End_Date_Or_Time TEXT, 
                y_creation_date timestamp, 
                y_update_date timestamp
            ) ON COMMIT PRESERVE ROWS;
            """
        ...
        if source_id in (904, 981):
            insert_temp_query = f"""
            INSERT INTO {temp_table}
            (source_id, batch_id, ListingKey, openhousedate, Start_Date_Or_Time, Event_End_Date_Or_Time, y_creation_date, y_update_date)
            VALUES (%(source_id)s, %(batch_id)s, %(SystemID)s, %(Start_Date_Or_Time)s, %(Start_Date_Or_Time)s, %(Event_End_Date_Or_Time)s, %(y_creation_date)s, %(y_update_date)s)
            """
        # Execute the insert
        try:
...
def download_all_Openhouse_func(
    ...
    Created:    2025-01-28                  Create By : Ammar Azkar |  Updated By : Fasihuddin
    """
    ...
    # -------------------------------------------------------------
    # 2. Build PARAGONRELS OpenHouse API endpoint and headers
    # -------------------------------------------------------------
    if source_id in (904, 981):
    ...
    if source_id in (904, 981):
        params = {
            "$filter": f"Start_Date_Or_Time ge {last_day_time}",
            "$select": "SystemID,Start_Date_Or_Time,Event_End_Date_Or_Time",
        }
    else:
        params = {
            "$filter": f"OpenHouseDate ge {last_day_time}",
    ...
    # -------------------------------------------------------------
    # 4. Call Paragonrels API and handle OData pagination
    # -------------------------------------------------------------
