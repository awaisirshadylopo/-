...
def download_all_openhouse(source_data, cursor_rds):
    ...
    select_fields = f"rets_oh_listing_id,rets_oh_start,rets_oh_end"

    current_datetime = datetime.now()
    one_day_ago = current_datetime - timedelta(days=1)
    one_day_ago_formatted = one_day_ago.strftime("%Y-%m-%dT%H:%M:%S.%f")

    hitting_query = f"(rets_oh_start={one_day_ago_formatted}+)"
