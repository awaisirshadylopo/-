"""MlsRouter API Validation and Data Download"""
...
# Add this new function for token management
def token_generation(data, source_id, cursor, connection):
    """Generate or refresh token as needed"""
    expires_in = data.get(
        "expires_in", "1970-01-01 00:00:00"
    )  # Default old date if not present
    now = datetime.now()
    ...
    # Otherwise create new token and update database
    else:
        ...
        future_date = now + timedelta(seconds=expires_in_seconds)
        ...
        connection.commit()
        response["expires_in"] = future_date_str
...
def download_all_Openhouse_func(
    ...
    Created:    2026-01-09                  Create By : Ammar Azkar
    """
    ...
    token_response = token_generation(auth, source_id, ls_cursor, homelisting_conn)

    token = token_response["access_token"]
    source_data["auth"]["access_token"] = token

    # -------------------------------------------------------------
    # 2. Build MlsRouter OpenHouse API endpoint and headers
    # -------------------------------------------------------------
    ...
    # -------------------------------------------------------------
    # 4. Call MlsRouter API and handle OData pagination
    # -------------------------------------------------------------
