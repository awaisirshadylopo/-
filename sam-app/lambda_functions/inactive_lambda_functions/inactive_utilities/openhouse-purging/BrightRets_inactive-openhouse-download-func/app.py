"""BRIGHT Inactive Lambda function for downloading and processing purge future OpenHouse records ."""
...
def data_download(data):
    ...
    while True:
        ...
        if count_element is not None:
            total_count = int(count_element.get("Records"))
        ...
        logger.info(
            f"Downloaded {len(all_rows)} / {total_count}"
        )
...
def process_openhouse_data(
    ...
    try:
        ...
        VALUES (%(source_id)s, %(batch_id)s, %({openhouse_key_column})s, %(openhousedate)s, %(openhousestarttime)s, %(openhouseendtime)s, %(y_creation_date)s, %(y_update_date)s)
        """
...
def download_openhouse_data(
    ...
    openhouse_key_column = event["source_info"].get("openhouse_key_column")
    logger.info({
            "source_id": source_id,
            "message": f"OpenHouseKey column {openhouse_key_column}",
        })
    if len(classes) == 0:
        log_msg = {
            "source_id": source_id,
    ...
    for chunk_index, listing_chunk in enumerate(active_listings_chunks):
        ...
        query_filter = f"({openhouse_key_column}={listing_chunk_str})"
        ...
        for class_name in classes:
            query_params = {
                ...
                "Class": class_name,
                "Query": query_filter,
                "Select": f"{openhouse_key_column},OpenHouseDate,OpenHouseStartTime,OpenHouseEndTime",
            }
    ...
    try:
        rows_processed = process_openhouse_data(
            ...
            openhouse_key_column=openhouse_key_column,
            source_id=source_id,
            batch_id=batch_id,
