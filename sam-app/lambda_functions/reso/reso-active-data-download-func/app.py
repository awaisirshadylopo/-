# Reso active data download lambda
...
logger = logging.getLogger("Reso-Active-Lambda")
...
def request_and_load_temp_table(source_data, flow_type, cursor_rds, rds_connection):
    ...
    media_modification_column = source_info["media_modification_column"]
    ...
    params = {
        ...
        "$select": f"ListingKey,ModificationTimestamp,{media_modification_column}",
    }

    if flow_type in ["full_load", "respecs", "sold"]:
        sold_status = source_info["sold_status"]
        filter_value = filter_value + f" and {status_column} ne {sold_status}"
    ...
    if len(temp_table_df) != 0:
        ...
        temp_table_df.rename(
            columns={
                ...
                f"{media_modification_column}": "media_modification_timestamp",
            },
            inplace=True,
        )
...
def download_func(source_data, cursor_homelisting, cursor_rds, rds_connection):
    ...
    try:
        ...
        office_mlsid_columns = source_info["office_mlsid_columns"]
        agent_mlsid_columns = source_info["agent_mlsid_columns"]
        office_key_columns = source_info["office_key_columns"]
        agent_key_columns = source_info["agent_key_columns"]
        ...
        for resource_name in resources:
            ...
            if resource_name == "Property":
                ...
                if len(property_df) != 0:
                    ...
                    for expanded_class in expand_classes:
                        ...
                        table_name = f"ps_reso_{expanded_class}_{source_id}"

                        if (
                            "media" in expanded_class.lower()
                            or "picture" in expanded_class.lower()
                        ):
                            table_name = f"ps_reso_media"
            ...
            if len(data_df) != 0:
                ...
                prestage_tables_insertion(
                    ...
                    source_name,
                    resource_name,
                    f"ps_reso_{resource_name}_{source_id}",
                    cursor_rds,
                    rds_connection,
                )
