"""Silvar Rets Active Data Download Lambda"""
    ...
    try:
        ...
        try:
            ...
            if "no records found" in reply_text.lower():  # type: ignore
                logger.warning(f"{reply_text} Warning. Resource: {query_params["SearchType"]}, Class: {query_params["Class"]}")
                return pd.DataFrame(), 0
...
def get_hitting_query(source_data, flow_type, class_name):
    ...
    if flow_type == "sold":
        ...
        return f"(({sold_column}={sold_date} - {current_date}),({status_column}=|{sold_status}))"

    else:
        current_datetime = current_datetime.strftime("%Y-%m-%d %H:%M:%S.%f")
        current_timestamp = formatted_date(current_datetime)
        
        if flow_type == "full_load" or flow_type == "respecs":
            active_status = data_source_info["active_status"]
            return f"(LastModDt={last_modified_date}-{current_timestamp}),({status_column}=|{active_status})"

        else:       
            return f"(LastModDt={last_modified_date}-{current_timestamp})"
...
def table_creation_and_loading(
    ...
    log_msg = {
        ...
        "insertion_count": len(df),
    }
    logger.info(log_msg)


def adding_extra_columns(generic_df, batch_creation_date, source_id, batch_id, formatted_datetime):
...
def download_temp_table(
    ...
    try:
        ...
        for class_name in get_classes:
            ...
            select = f"PropertyID,LastModDt,PhotoModificationTimestamp"
            if flow_type == "sold":
                sold_column = source_data["source_info"]["sold_column"]
                select = select + f",{sold_column}"

            hitting_query = get_hitting_query(source_data, flow_type, class_name)
            ...
            data_df.rename(
                columns={
                    ...
                    "LastModDt": "modification_timestamp",
                    "PhotoModificationTimestamp": "media_modification_timestamp",
                },
                inplace=True,
        ...
        if len(temp_table_df) != 0:
            ...
            temp_table_df.drop(columns=["search_url", "query_params"], inplace=True, axis=1)
    ...
    for _, row in data_for_download.iterrows():
        ...
        if resource_name == "Property":
            ...
            if len(data_df) != 0:
                agent_ids_chunks.extend(
                    chunks_creation(
                        data_df, ["ListingAgentID", "coListingAgentID", "SellingAgentID", "coSellingAgentID"], chunk_size
                    )
                )

                office_ids_chunks.extend(
                    chunks_creation(data_df, ["ListingOfficeID", "coListingOfficeID", "SellingOfficeID", "coSellingOfficeID"], chunk_size)
                )
        ...
        if len(data_df) != 0:
            ...
            data_df = adding_extra_columns(
                data_df, batch_creation_date, source_id, batch_id, formatted_datetime
            )
...
def validation_func(serverless_db_con, cursor_serverless, cursor_pentaho, source_data):
    ...
    try:
        ...
        log_message = {
            ...
            "table_name": "idx_stage.temp_table"
        }
