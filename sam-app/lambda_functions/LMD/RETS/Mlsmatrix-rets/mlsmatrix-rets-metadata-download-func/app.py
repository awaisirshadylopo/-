...
logger = logging.getLogger("mls-matrix-rets-metadata-download")
...
def login(data):
    ...
    username = data["user"]
    if source_id == 267:
        headers = {"User-Agent": "RETSMD/1.0", "RETS-Version": "RETS/1.5"}
    else:
        headers = {"User-Agent": "RETS-Connector/1.2", "RETS-Version": "RETS/1.5"}
    session = requests.Session()

    # Create a session
    session = requests.Session()
    response = session.get(
        loginUrl, headers=headers, auth=HTTPDigestAuth(username, password)
    )
    ...
    try:
        ...
        logger.error("Login processing failed: %s", log_msg)
        raise Exception(log_msg) from e
...
def class_metadata(data, df):
    ...
    for row in df.iterrows():
        ...
        try:
            ...
            logger.error("Unable to download clss_metadata: %s", log_msg)
            raise Exception(log_msg) from e
...
def field_metadata(data, df):
    ...
    for row in df.iterrows():
        ...
        try:
            ...
            logger.info(f"Data Download for Resource: {resource}")
            df_temp.insert(0, "class_name", class_name)
            df_temp.insert(0, "resource_name", resource)
            df_temp.insert(0, "source_name", source_name)
            df_temp.insert(0, "source_id", source_id)
            field_df = pd.concat([field_df, df_temp], ignore_index=True)
            ...
            logger.error("Unable to download field_metadata: %s", log_msg)
            raise Exception(log_msg) from e

    # Renaming columns - only rename if they exist
    renaming_columns = {
        "LongName": "Long_Name",
        "DBName": "DB_Name",
    ...
    existing_renames = {
        k: v for k, v in renaming_columns.items() if k in field_df.columns
    }
    field_df = field_df.rename(columns=existing_renames)

    # Add renamed_long_name column if LongName exists
    if "Long_Name" in field_df.columns:
        field_df["renamed_long_name"] = field_df["Long_Name"]
    elif "LongName" in field_df.columns:
        field_df["renamed_long_name"] = field_df["LongName"]

    # Define the desired columns in order of preference
    desired_columns = [
        "source_id",
        "source_name",
        "resource_name",
    ...
    available_columns = [col for col in desired_columns if col in field_df.columns]
    field_df = field_df[available_columns]
    ...
    # Drop duplicates only if Long_Name column exists
    if "Long_Name" in field_df.columns:
        field_df = field_df.drop_duplicates(
            subset=["Long_Name", "resource_name"], keep="first"
        )
...
def rename(source_id, cursor_rds):
    ...
    qurey = "select id, source_id, long_name, renamed_long_name , rename_flag,  status_flag from dev.field_metadata where Rename_flag= false and source_id = {}".format(
        source_id
    )
...
def update_rename_in_db(cursor_rds, df):
    ...
    try:
        ...
        logger.error("Failed in update_rename_in_db: %s", log_msg)
        raise Exception(log_msg) from e
...
def ddl_generation(source_id, connection, cursor):
    ...
    ps_tables = """select distinct resource_name from dev.class_metadata where source_id = {} and download_flag = true ;""".format(
        str(source_id)
    )
    ...
    for res_name in ps_tables:
        final_names = ""
        if res_name[0] == "Property":  # Fixed: use == instead of 'in'
            res_name = res_name[0]
            long_names = """ select distinct lower(long_name) from dev.field_metadata where source_id = {0} and resource_name = '{1}' and active_flag = true order by lower(long_name); """.format(
                str(source_id), res_name
            )
            ...
            final_query = """CREATE TABLE IF NOT EXISTS idx_stage.ps_rets_{0}_{1} ( {2} ); """.format(
                res_name, source_id, final_names
            )
        ...
        logger.info(f"Executing DDL: {final_query}")

        try:
            cursor.execute(final_query)
            logger.info(
                f"Table created successfully: idx_stage.ps_rets_{res_name}_{source_id}"
            )
        except Exception as e:
            log_msg = {
                "Level": "Error",
                "Function": "ddl_generation()",
                "Error": e,
                "Source": source_id,
            ...
            logger.error(
                "Error creating table idx_stage.ps_rets_{res_name}_{source_id}: %s",
                log_msg,
            )
            raise Exception(log_msg) from e

    connection.commit()
    logger.info("DDL generation completed and committed")
    ...
    try:
        ...
        for i, item in source_list.iterrows():
            ...
            if rets_data:
                ...
                metadata_change_detection_and_updation(
                    rets_data["source_id"], cursor_serverless
                )
                serverless_db_con.commit()
                ...
                if event["ddl_generation"]:
                    ddl_generation(
                        rets_data["source_id"], serverless_db_con, cursor_serverless
                    )
