"""MlxMatrix Rets Active Data Download Lambda"""
...
def get_hitting_query(source_data, flow_type):
    ...
    photo_timestamp_column = data_source_info["photo_timestamp_column"]
    ...
    if flow_type == "sold":
        ...
        return f"({sold_column}={sold_date}+),({modification_column}={last_modified_date}+),({status_column}=|{sold_status}),~(PropertyType=|ResidentialLease,CommercialLease)"
        ...
        return f"(({modification_column}={last_modified_date}+)|({photo_timestamp_column}={last_media_modified_date}+))"
...
def download_data_for_given_list(
    ...
    for sub_chunk in chunks_list:
        limit = len(sub_chunk)
        # filtered_values = list(filter(None, sub_chunk))
...
def download_temp_table(
    ...
    for class_name in get_classes:
        ...
        if count == 0:
            continue
        logger.info(f"count of the {class_name} is {count}")
        top = 1000
        skip = 0
...
def request_and_load_tables(
    ...
    room_key_column = source_data["source_info"]["room_key_column"]
    unit_key_column = source_data["source_info"]["propertyunityypes_key_column"]
    openhouse_key_column = source_data["source_info"]["openhouse_key_column"]
    office_key_column = source_data["source_info"]["office_key_column"]
    office_columns = source_data["source_info"]["office_columns"]
    agent_key_column = source_data["source_info"]["agent_key_column"]
    agent_columns = source_data["source_info"]["agent_columns"]
    key_column = source_data["source_info"]["key_column"]
    ...
    query = f""" select
        ...
        listingkey::bigint as listingkey
    ...
    for _, row in data_for_download.iterrows():
        ...
        if resource_name == "Property":
            for listing_chunk in listings:
                listing_ = [str(value) for value in listing_chunk]
                listing_str = ",".join(listing_)
                query = f"({key_column}={listing_str})"
            ...
            if not data_df.empty:
                if (
                    key_column in data_df.columns
                    and photo_count_column in data_df.columns
                ):
                    media_download_set = pd.concat(
                        [media_download_set, data_df[[key_column, photo_count_column]]]
                    )

            # Extract agent and office keys - check if columns exist first

            # Only try to extract keys if we have data and the columns exist
            if not data_df.empty:
                available_agent_columns = [
                    col for col in agent_columns if col in data_df.columns
                ]
                available_office_columns = [
                    col for col in office_columns if col in data_df.columns
                ]

                if available_agent_columns:
                    member_keys_chunks = chunks_creation(
                        data_df, available_agent_columns, chunk_size
                    )
                if available_office_columns:
                    office_keys_chunks = chunks_creation(
                        data_df, available_office_columns, chunk_size
                    )

        elif resource_name == "Agent" or resource_name == "Member":
            ...
            for listing_chunk in listings:
                ...
                query = f"({room_key_column}={listing_chunk})"
            ...
            for listing_chunk in listings:
                listing_chunk = str(listing_chunk).replace("[", "").replace("]", "")
                query = f"({unit_key_column}={listing_chunk})"
            ...
            for listing_chunk in listings:
                listing_chunk = str(listing_chunk).replace("[", "").replace("]", "")
                query = f"({openhouse_key_column}={listing_chunk})"
    ...
    if not media_download_set.empty:
        get_object_url = response.get("GetObject")
        resource_name = "Media"

        if get_object_url:
            media_df = photo_count(media_download_set, get_object_url, source_data)
            target_table = "ps_rets_media"
            ...
            log_msg = {
                "source_id": source_id,
                "Records Inserted": len(media_df),
                "Table Name": target_table,
                "Resource Name": f"{resource_name} Data Download Successfully",
            }
            logger.info(log_msg)
        else:
            log_msg = {
                "Message": "GetObject URL not found in response, skipping media download",
                "source_id": source_id,
                "source_name": source_name,
                "resource_name": resource_name,
            }
