...
logger = logging.getLogger("mls-flexmls-rets-metadata-download")
...
def resource_metadata(data):
    ...
    #metadata_url="http://retsgw.flexmls.com/rets2_3/GetMetadata"
    data_login=data['loginUrl']
    login_url = re.sub(r":\d+", "",data_login).split("/")[0] + "//" + re.sub(r":\d+", "", data_login).split("/")[2]
    metadata_url = login_url +  data['GetMetadata']
...
def field_metadata(data, df):
    ...
    login_url = re.sub(r":\d+", "",data_login).split("/")[0] + "//" + re.sub(r":\d+", "", data_login).split("/")[2]
    metadata_url = login_url +  data['GetMetadata']
    print(metadata_url,'here metadata_url')
...
def lambda_handler(event, context):
    ...
    auth=event.get('auth')
    # loginUrl=event.get['auth']['loginUrl']
    try:
