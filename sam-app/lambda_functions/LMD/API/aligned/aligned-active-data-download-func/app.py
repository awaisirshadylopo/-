# aligned active data download lambda
...
logger = logging.getLogger("aligned-Active-Lambda")
...
def download_data_for_chunks(
    ...
    if len(key_chunks) > 0:
        ...
        for each_chunk in key_chunks:
            ...
            each_chunk = [str(v).replace("&", "%26") for v in each_chunk]
            params = {
                "$filter": "{} in ({})".format(
                    key_column,
...
def request_and_load_temp_table(source_data, flow_type, cursor_rds, rds_connection):
    ...
    if flow_type in ["full_load", "respecs"]:
        ...
        filter_value = filter_value + f" and ({active_status})"
    ...
    extra_filter = " and PropertyType ne 'Rental'" if source_id != 1025 else ""
...
def mls_ids_case(source_id, cursor_rds, rds_connection, homelisting_cursor, sql_case):
    ...
    rds_query = """SELECT ListingId, ListingKey FROM idx_stage.ps_aligned_property_{} ORDER BY 1, 2;""".format(
        source_id
    )
    ...
    if final_list:
        update_query = """ 
            UPDATE idx_stage.ps_aligned_property_{0}
                SET ListingId = concat(ListingId, '_', ({1}) )
            WHERE source_id = {0} AND ListingId IN {2}
            """.format(
            source_id, sql_case, str(final_list).replace("[", "(").replace("]", ")")
        )
        ...
        for resource_name in resources:
            ...
            if resource_name == "Property":
                ...
                if len(property_df) != 0:
                    ...
                    for expanded_class in expand_classes_list:
                        ...
                        table_name = f"ps_aligned_{expanded_class}_{source_id}"
                        # if expanded_class == "Media":
                        #     table_name = f"ps_aligned_{expanded_class}"
            ...
            if len(data_df) != 0:
                ...
                prestage_tables_insertion(
                    ...
                    source_name,
                    flow_type,
                    f"ps_aligned_{resource_name}_{source_id}",
                    cursor_rds,
                    rds_connection,
                )
