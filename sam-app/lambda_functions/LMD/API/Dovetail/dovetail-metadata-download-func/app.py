...
def creds_from_source_table(connection, cursor, sid):
    creds = f"select id, name, auth->'loginUrl' as loginurl, auth->'password' as token, auth->'user' as user, source_info->'token_generation' as token_generation from public.source where source_info::text ~* 'Dovetail' and id = {sid};"
    # creds = f"select id, name, auth->'metadataUrl' as loginurl, auth->'password' as token from public.source where source_info::text ~* 'MLS Grid' and id = {sid};"
    ...
    for all_tree in root:
        for inner_tree in all_tree:
            if 'Namespace' in inner_tree.attrib and inner_tree.attrib[
                'Namespace'] == 'com.dovetail.metadata':
...
def ddl_generation(source_id, connection, cursor):
    ...
    for each_table in ps_tables_name:
        ...
        table = f"idx_stage.ps_dovetail_{each_table}_{source_id}" if each_table != 'Media' else f"idx_stage.ps_dovetail_{each_table}"
...
def lambda_handler(event, context):
    try:
        ...
        client_id = credentials[0][4]
        password = credentials[0][3]
        token_generation = credentials[0][5]

        # GENERATES TOKEN
        if token_generation:
            password = create_token(source_id, source_name, client_id, password)
        
        classList, list_dict, status_code = DownloadMetaData(credentials[0][2], password)


        metadata_insertion(classList, list_dict, source_name, credentials[0][0], rds_conn, rds_cursor)
